...
module Telstra_EventDetection
  class RegistrationApi
    ...
    # @param body Subscribe with a list of phone numbers, push notification URL (optional) and the event type.
    ...
    # @param body Subscribe with a list of phone numbers, push notification URL (optional) and the event type.
    # @param [Hash] opts the optional parameters
    # @return [Array<(ResisterPhoneNumberList, Fixnum, Hash)>] ResisterPhoneNumberList data, response status code and response headers
    ...
    # @param body List of subscribed phone numbers and notification URL (optional)
    # @param [Hash] opts the optional parameters
    # @return [ResisterPhoneNumberList]
    ...
    # @param [Hash] opts the optional parameters
    # @return [Array<(ResisterPhoneNumberList, Fixnum, Hash)>] ResisterPhoneNumberList data, response status code and response headers
    def unregister_with_http_info(event_type, body, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: RegistrationApi.unregister ..."
      end
      # verify the required parameter 'event_type' is set
      if @api_client.config.client_side_validation && event_type.nil?
        fail ArgumentError, "Missing the required parameter 'event_type' when calling RegistrationApi.unregister"
      end
      # verify enum value
      if @api_client.config.client_side_validation && !['simswap'].include?(event_type)
        fail ArgumentError, "invalid value for 'event_type', must be one of simswap"
      end
      # verify the required parameter 'body' is set
      if @api_client.config.client_side_validation && body.nil?
        fail ArgumentError, "Missing the required parameter 'body' when calling RegistrationApi.unregister"
      end
      # resource path
      local_var_path = "/v1/eventdetection/events/{eventType}".sub('{' + 'eventType' + '}', event_type.to_s)
      ...
      data, status_code, headers = @api_client.call_api(:DELETE, local_var_path,
        ...
        :return_type => 'ResisterPhoneNumberList')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: RegistrationApi#unregister\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
